...
const PopularTestSeries = () => {
  return (
    <div className="bg-[#f5f8ff] h-[600px] py-12 px-4 text-center">
      <h2 className="text-3xl font-bold mb-10 text-start pl-[200px]">Popular Test Series</h2>

<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl h-[300px] mx-auto">
  {testSeries.map((series) => (
    <div
      key={series.id}
      className="bg-gradient-to-b from-purple-100 to-white p-6 rounded-xl shadow hover:shadow-lg transition flex flex-col justify-between h-full"
    >
      <div>
        ...
        <h3 className="font-semibold text-lg mb-1 text-start">{series.title}</h3>
        <p className="text-sm text-gray-600 mb-2 text-start">
          <strong>{series.totalTests} Total Tests</strong> |{" "}
          <span className="text-green-600 text-start">{series.freeTests} Free Tests</span>
        </p>
      ...
      <button className="mt-16 px-6 py-2 border border-sky-500 text-sky-500 rounded hover:bg-sky-600 transition">
